...
def load_test_samples(data_dir, num_samples=10, split='test', split_file=None):
    """
    加载测试样本
    ...
    Args:
        ...
        split_file: 划分文件路径（可选，默认在data_dir下查找）
    ...
    if split_file and os.path.exists(split_file):
        ...
        with open(split_file, 'r') as f:
            splits = json.load(f)
            test_ids = set(splits.get(split, []))
    else:
        # 方法2: 检查data_dir下的常见划分文件
        possible_split_files = [
            os.path.join(data_dir, 'splits.json'),
            os.path.join(data_dir, 'split.json'),
            os.path.join(data_dir, 'data_split.json'),
            os.path.join(data_dir, f'{split}_ids.txt'),
            os.path.join(data_dir, f'{split}.txt'),
        ]
        
        for sf in possible_split_files:
            if os.path.exists(sf):
                ...
                if sf.endswith('.json'):
                    with open(sf, 'r') as f:
                        splits = json.load(f)
                        if split in splits:
                            test_ids = set(splits[split])
                        elif f'{split}_ids' in splits:
                            test_ids = set(splits[f'{split}_ids'])
                elif sf.endswith('.txt'):
                    with open(sf, 'r') as f:
                        test_ids = set(line.strip() for line in f if line.strip())
                break
                
    # 方法3: 检查description.csv是否有split列
    if test_ids is None:
        print(f"   未找到划分文件，检查description.csv中是否有split列...")
        with open(desc_file, 'r') as f:
            ...
            for idx, col in enumerate(header):
                if col.lower() in ['split', 'dataset', 'set', 'partition']:
                    split_col_idx = idx
                    break
                    
            if split_col_idx is not None:
                print(f"   在description.csv第{split_col_idx}列找到划分信息")
                test_ids = set()
                f.seek(0)
                next(reader)  # skip header again
                for row in reader:
                    if len(row) > split_col_idx and row[split_col_idx].lower() == split.lower():
                        test_ids.add(row[0])
    ...
    if test_ids is not None and len(test_ids) > 0:
        print(f"   {split}集共有 {len(test_ids)} 个样本")
        data = [row for row in all_data if row[0] in test_ids][:num_samples]
    else:
        print(f"   ⚠️ 未找到划分信息，使用最后 {num_samples} 个样本作为测试集")
        # 使用最后的样本（通常划分时测试集在最后）
        data = all_data[-num_samples:] if len(all_data) > num_samples else all_data
        
    print(f"   选择 {len(data)} 个样本进行分析")
    ...
    print(f"✅ 加载了 {len(samples)} 个{split}集样本")
    return samples
...
def main():
    ...
    parser.add_argument('--split_file', type=str, default=None,
                        help='数据集划分文件路径（JSON格式，包含train/val/test键）')
